...
url = "https://www.localgovernment.qld.gov.au/for-councils/grants/current-programs"
...
if response.status_code == 200:
    ...
    resultContainer = soup.find('div', class_="main-content")
    resultCardHtml = resultContainer.find_all("h2") if resultContainer else []
...
for resultCard in resultCardHtml:
    ...
    description = resultCard.find_next_sibling("p").get_text(strip=True) if resultCard.find_next_sibling("p") else "N/A"
...
collection = db["qld_local"]  # You can change the collection name if you want
